...
const ThreeJSAnimation = ({ className }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Try to create a WebGL context to test support
      ...
      const renderer = new THREE.WebGLRenderer({ 
        antialias: true, 
        alpha: true,
        powerPreference: 'default', // Adding power preference for better compatibility
        failIfMajorPerformanceCaveat: false // Allow lower performance contexts
      });
  ...
  // If there's an error, render a gradient background as fallback
  if (error) {
    return (
      <div 
        className={`w-full h-full bg-gradient-to-b from-uga-red to-uga-black ${className}`}
      />
    );
  }
